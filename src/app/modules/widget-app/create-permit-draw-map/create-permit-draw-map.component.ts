import { Component, NgZone, OnInit, Input, OnChanges, SimpleChanges, Output, EventEmitter, ViewChild } from '@angular/core';
import { stringToKeyValue } from '@angular/flex-layout/extended/typings/style/style-transforms';
import { MatDialog } from '@angular/material/dialog';
import { NAVIGATION_DATA, GLOBAL_PARAM_KEY } from 'src/app/app-constants';
...
import { WidgetConstants, FormModeConstant } from 'src/app/constants/widget-constants';
import { MASTER_DATA_DOMAIN, MASTER_DATA, ELGIN_MAP_GEOMETRY_TYPE, ONE_NETWORK_GEOMETRY_TYPE } from 'src/app/constants/app-repo.constants';
    ...
    this._elginService.getElginToken().then((response) => {
      this._elginService.reverseGEOCode(requestData).then((response: any) => {
        ...
        if (requestData.geom_type === ONE_NETWORK_GEOMETRY_TYPE.POLYGON) {
          ...
          let arrUsrns = response?.affected?.usrns?.split(',');
          let responseModel = this.convertData(response);
          this.setValue(responseModel);
          if (arrUsrns && arrUsrns.length == 1) {
...
if(response.road_category==9){
  ...
  validate() {
    if (this.dataModel.data?.usrn && this.dataModel.data.road_category) {
